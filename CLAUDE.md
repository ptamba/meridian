...
tools/
  ...
  coin-filters.js   Deterministic "avoid" filters (genre blacklist, mint/freeze authority) — pure fns shared by the live screener and the backtest
```
...
| genreBlacklistEnabled | screening | true — filter slow-rug genres (political/celebrity/justice) via `tools/coin-filters.js matchGenre`, in `getTopCandidates` (before LLM) + a deploy-time block |
...
- Genre blacklist (`genreBlacklistEnabled`, default on): `getTopCandidates()` drops political/celebrity/justice-narrative coins before the LLM sees them (`coin-filters.js matchGenre`); `deploy_position` re-checks `args.pool_name` as belt-and-suspenders for direct/manual deploys
...
- Each closed-position row also stores `peak_pnl_pct` and `max_drawdown_pct` (the intra-hold PnL extremes; `max_drawdown_pct` is the running most-negative trustworthy PnL, tracked per poll in `state.js updatePnlAndCheckExits`, passed through both `dlmm.js` close paths, persisted in `lessons.js`). These exist so stop-level / early-exit changes can be backtested against real paths — the final close value alone can't say which winners dipped past a candidate stop then recovered. Rows recorded before this shipped, or positions already open at the restart that added it, have shallow/null values (only deploys opened after the restart are clean).
...
- Backtest at `scripts/backtest-filters.js` measures proposed "avoid" filters against `lessons.json` **before** wiring any into the live screener, using the shared pure fns in `tools/coin-filters.js`. Per filter it reports positions blocked, losses avoided ($), winners killed ($), NET impact (ship only if > 0), stop-loss $ recovered, and funnel-removed % (starvation check). Filters: **genre** (pure) and **mint/freeze authority** (RPC, decrypts `.env` via a side-effect `import "../envcrypt.js"` like `index.js`; found redundant — Meteora already blocks unrevoked authorities). `--stop-sensitivity` prints a **savings-side-only** (upper-bound) estimate of tightening the stop, which the two-sided answer needs `max_drawdown_pct` to complete. Flags: `--no-rpc` (genre only, instant), `--since`, `--file`, `--json`. creator≠minter is scaffolded (phase 2); dev-% isn't backtestable (deploy-time holder distribution not stored).
