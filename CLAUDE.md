...
- Each closed-position row also stores `peak_pnl_pct` and `max_drawdown_pct` (the intra-hold PnL extremes, tracked per poll in `state.js updatePnlAndCheckExits`). These exist so stop-level / early-exit changes can be backtested against real paths — the final close value alone can't say which winners dipped past a candidate stop then recovered. `scripts/backtest-filters.js` backtests proposed "avoid" filters (genre + mint/freeze authority) against `lessons.json` and, with `--stop-sensitivity`, prints a savings-side-only tighter-stop estimate. Rows recorded before this shipped have neither field (null).
