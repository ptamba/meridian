  ...
  try {
    ...
    if (success && !result?.dry_run) {
      if (name === "swap_token" && result.tx) {
        ...
        notifyClose({ pair: result.pool_name || args.position_address?.slice(0, 8), pnlUsd: result.pnl_usd ?? 0, pnlPct: result.pnl_pct ?? 0, reason: args.reason || result.close_reason }).catch(() => {});
